...
/* Author: LionStyle*/
/* Website: http://moonart.net.ua/Vega/ 
/* Copyright: (C) 2016 */
/*-------------------------------------------------------------------------------------------------------------------------------*/
...
$(function () {
  ...
  function scrollCall() {
    ...
    if ($(".homepage-4-slider-navigation").length) {
      if (winScr + winH - 115 >= footerTop)
        $(".homepage-4-slider-navigation .rotate").css({
          "margin-top": -1 * (winScr + winH - footerTop - 115),
        });
      else
    ...
    if (event.deltaY > 0) {
      swipers[
        "swiper-" +
          $(
            '.swiper-container[data-pagination-rel="' +
              $(this).find(".slider-click-label").data("pagination-rel") +
              '"]'
          ).attr("id")
      ].slidePrev();
    } else {
  ...
  $(".tab-menu").on("click", function () {
    ...
    $(this)
      .closest(".sorting-menu")
      .find(".responsive-filtration-title .text")
      .text($(this).text());
    $(
      '.tab-entry[data-tab-menu="' + $this.data("tab-menu") + '"]:visible'
    ).animate({ opacity: "0" }, function () {
      $(this).hide();
      var tab = $(
        '.tab-entry[data-tab-menu="' +
          $this.data("tab-menu") +
          '"][data-tab="' +
          $this.data("tab") +
          '"]'
